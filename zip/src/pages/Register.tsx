    ...
    <div className="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display min-h-screen flex flex-col">
      <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-6 lg:px-10 py-4 shadow-sm">
        ...
        <div className="hidden md:flex flex-1 justify-end gap-8 items-center">
          ...
          <Link to="/login" className="flex min-w-[84px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-6 bg-primary/10 hover:bg-primary/20 text-primary dark:text-blue-400 text-sm font-bold leading-normal tracking-wide transition-colors">
      ...
      <main className="flex-grow flex items-center justify-center p-4 py-12">
        <div className="w-full max-w-2xl bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800 overflow-hidden">
          <div className="px-8 pt-10 pb-6 text-center">
            <h1 className="text-3xl lg:text-4xl font-black text-slate-900 dark:text-white mb-3 tracking-tight">Đăng ký Giáo viên</h1>
            <p className="text-slate-500 dark:text-slate-400 text-lg">Tạo tài khoản để bắt đầu quản lý lớp học hiệu quả hơn</p>
          </div>
          <form className="px-6 lg:px-12 py-6 flex flex-col gap-6">
            <div className="flex flex-col gap-2">
              <label className="text-slate-900 dark:text-slate-100 text-base font-bold" htmlFor="fullname">Họ và tên</label>
              <input className="form-input w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:border-primary focus:ring-2 focus:ring-primary/20 h-14 px-4 text-lg placeholder:text-slate-400 dark:placeholder:text-slate-500 transition-all" id="fullname" placeholder="Nguyễn Văn A" type="text" />
            </div>
            <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-slate-900 dark:text-slate-100 text-base font-bold" htmlFor="confirm-password">Xác nhận mật khẩu</label>
                <div className="relative">
                  <input className="form-input w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:border-primary focus:ring-2 focus:ring-primary/20 h-14 pl-4 pr-12 text-lg placeholder:text-slate-400 dark:placeholder:text-slate-500 transition-all" id="confirm-password" placeholder="******" type="password" />
                  <button className="absolute right-0 top-0 h-full px-4 text-slate-400 hover:text-primary transition-colors flex items-center justify-center" type="button">
                    <span className="material-symbols-outlined">visibility_off</span>
                  </button>
                </div>
            ...
            <div className="flex flex-col gap-2 mt-2">
              <label className="text-primary dark:text-blue-400 text-base font-bold flex items-center gap-2" htmlFor="invite-code">
                <span className="material-symbols-outlined text-xl">vpn_key</span>
                Mã mời (Bắt buộc)
              </label>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <span className="material-symbols-outlined text-primary/60 dark:text-blue-400/60">key</span>
                </div>
                <input className="form-input w-full rounded-lg border-2 border-primary/20 dark:border-blue-500/20 bg-primary/5 dark:bg-blue-900/10 text-slate-900 dark:text-white focus:border-primary focus:ring-0 h-16 pl-12 px-4 text-xl font-medium tracking-wide placeholder:text-primary/40 dark:placeholder:text-blue-400/40 transition-all" id="invite-code" placeholder="NHAP-MA-MOI" type="text" />
              </div>
              <p className="text-sm text-slate-500 dark:text-slate-400 pl-1">Nhập mã được cung cấp bởi nhà trường hoặc quản trị viên.</p>
            </div>
            <div className="flex items-start gap-3 mt-2">
              <input className="mt-1.5 size-5 rounded border-slate-300 text-primary focus:ring-primary/20 dark:border-slate-600 dark:bg-slate-800" id="terms" type="checkbox" />
              <label className="text-base text-slate-600 dark:text-slate-300" htmlFor="terms">
          ...
          <div className="px-8 pb-10 pt-2 text-center">
            <p className="text-base text-slate-600 dark:text-slate-400">
              Đã có tài khoản?
              <Link className="text-primary dark:text-blue-400 font-bold hover:underline ml-1 text-lg" to="/login">Đăng nhập tại đây</Link>
            </p>
          </div>
        </div>
      </main>
